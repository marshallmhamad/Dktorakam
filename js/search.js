...
const API_ENDPOINT =
  "https://taranjeetsinghkalsi.github.io/Find-My-Doctor/doctorsdata.json";
  ...
  .then((data) => {
    ...
    data.forEach((doctor) => {
      ...
      doctorCard.querySelector(".doctor-location").textContent =
        doctor.location;
      doctorCard.querySelector(".doctor-directions").href = doctor.directions;
      doctorCard.querySelector(".doctor-phone").href = doctor.phone;
...
specialitySearchInput.addEventListener("input", (e) => {
  ...
  doctors.forEach((doctor) => {
    if (doctor.classList.contains(value)) doctor.classList.add("visible");
...
areaSearchInput.addEventListener("input", (e) => {
  ...
  specialDoctors.forEach((specialDoctors) => {
    var areaName = specialDoctors
      .querySelector(".area-name")
      .textContent.toLowerCase();
    if (areaName.includes(value)) {
      specialDoctors.classList.add("visible");
      noResult = false;
    } else specialDoctors.classList.remove("visible");
  });
